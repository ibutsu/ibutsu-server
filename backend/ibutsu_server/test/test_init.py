...
class TestMaybeSqlUrl:
    """Test the maybe_sql_url function"""

    def test_maybe_sql_url_with_all_params(self):
    ...
    def test_maybe_sql_url_with_hostname_and_db(self):
...
class TestMakeCeleryRedisUrl:
    """Test the make_celery_redis_url function"""

    def test_make_celery_redis_url_with_envvar(self):
    ...
    def test_make_celery_redis_url_with_password(self):
    ...
    def test_make_celery_redis_url_without_password(self):
    ...
    def test_make_celery_redis_url_missing_hostname(self):
    ...
    def test_make_celery_redis_url_missing_port(self):
...
class TestGetApp(BaseTestCase):
    """Test the get_app function and application factory"""
    ...
    def test_get_app_with_extra_config(self):
    ...
    def test_get_app_loads_environment_variables(self):
        """Test that app loads configuration from environment variables"""
        app = get_app(TESTING=True, SQLALCHEMY_DATABASE_URI="sqlite:///:memory:")

        assert app.config["TEST_ENV_VAR"] == "env_value"

    def test_get_app_user_login_enabled_string_true(self):
    ...
    def test_get_app_user_login_enabled_string_false(self):
    ...
    def test_get_app_user_login_enabled_boolean(self):
    ...
    def test_get_app_superadmin_creation(self, mock_add_superadmin):
        ...
        app = get_app(
            TESTING=True, SQLALCHEMY_DATABASE_URI="sqlite:///:memory:", **superadmin_config
        )
...
class TestAppRoutes(BaseTestCase):
    """Test application routes defined in __init__.py"""
    ...
    def test_run_task_route_no_params(self):
        """Test run-task route with no parameters"""
        response = self.client.post("/admin/run-task")

        assert response.status_code == HTTPStatus.BAD_REQUEST

    def test_run_task_route_no_token(self):
    ...
    def test_run_task_route_invalid_token(self):
    ...
    def test_run_task_route_non_superadmin(self):
        """Test run-task route with non-superadmin user"""
        # Use the test user token (non-superadmin)
        response = self.client.post(
            "/admin/run-task",
            data=json.dumps({"token": self.jwt_token, "task": "test.task"}),
            content_type="application/json",
        )
    ...
    def test_run_task_route_no_task(self, mock_decode_token, mock_user):
    ...
    def test_run_task_route_module_not_found(
        self, mock_import_module, mock_decode_token, mock_user
    ):
    ...
    @patch("ibutsu_server.decode_token")
    @patch("ibutsu_server.import_module")
    def test_run_task_route_task_not_found(self, mock_import_module, mock_decode_token, mock_user):
    ...
    @patch("ibutsu_server.decode_token")
    @patch("ibutsu_server.import_module")
    def test_run_task_route_success(self, mock_import_module, mock_decode_token, mock_user):
    ...
    def test_run_task_route_invalid_json(self):
        """Test run-task route with invalid JSON"""
        response = self.client.post(
            "/admin/run-task", data="invalid json", content_type="application/json"
        )
    ...
    @patch("ibutsu_server.User")
    @patch("ibutsu_server.decode_token")
    def test_run_task_route_user_not_found(self, mock_decode_token, mock_user):
    ...
    @patch("ibutsu_server.decode_token")
    def test_run_task_route_token_decode_no_sub(self, mock_decode_token):
